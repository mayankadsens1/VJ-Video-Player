...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        name = await client.ask(message.chat.id, "<b>Welcome To Warrior Video Player.\n\nNow Create Account On Warrior Video Player to Continue\n\nNow Send Me Your Name Which Show On Website\nEx :- <code>Warrior Video Player</code></b>")
        if name.text:
...
async def update(client, message):
    vj = True
    if vj:
        name = await client.ask(message.chat.id, "<b>Now Send Me Your Name Which Show On Website\nEx :- <code>Warrior Video Player</code>\n\n/cancel - cancel the process</b>")
        if name.text == "/cancel":
